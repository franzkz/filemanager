...
function shUploader() {
	$('#uploader').dialog({
	    buttons: {
			"Закрыть": function() { uploader(); $(this).dialog("close"); }
		},
		width: 450,
		height: 470
	});
...
function getfInfo(fname, i) {
	...
	$("#dopenfile").html("<b>Зегружен: </b><a style='color: blue' href='" + url + "attach/?did=" + did + "&filename=" + encodeURIComponent(fname) + "' id='fdname'>" + fname + "</a>");
...
function createDirDialog() {
	$('<div title="Новый каталог" id="diradddialog">Имя каталога:&nbsp;<input type="text" name="dirname" id="fm_dirname" /></div>').dialog({
		modal: true,
	    buttons: {
	    	"Создать": function() {
	    		createDir();
...
function createDir() {
    ...
    if (_dirName.length == 0) {
    	$('<div title="Notify">Имя каталога пусто!</div>').dialog({
    		modal: true,
    	    buttons: {
        ...
        $.ajax({
        	...
        	success: function(res) {
        		if (res == "error") {
        			$('<div title="Error">Каталог с таким именем уже существует!</div>').dialog({
        	    		modal: true,
        	    	    buttons: {
        	                "Закрыть": function() { $(this).dialog("close"); }
        	    		}
...
function fileRename(name) {
	$("<div title='Переименовать файл'>Имя файла:&nbsp;<input type='text' name='dirname' id='fm_dirname' value='" + name + "' /></div>").dialog({
		modal: true,
	    buttons: {
	    	"OK": function() { 
	    		$.ajax({
...
function dirRename(did) {
	...
	$("<div title='Переименовать каталог'>Имя каталога:&nbsp;<input type='text' name='dirname' id='fm_dirname' value='" + name + "' /></div>").dialog({
		modal: true,
	    buttons: {
	    	"OK": function() { 
	    		$.ajax({
...
function delUserConfirm(uid) {
	$("<div title='Удалить'>Удалить пользователя?</div>").dialog({
		modal: true,
	    buttons: {
...
function delGroupConfirm(id) {
	$("<div title='Удалить'>Удалить группу?</div>").dialog({
		modal: true,
	    buttons: {
