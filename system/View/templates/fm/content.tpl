<div id="fmTop">
	<div style="float: left; margin-right: 20px"><b>Текущий каталог: [{{ shPath }}]</b></div>
	<div style="float: left">
		<div id="servSel">
			<div id="fm_sel" class="btn"><img style="vertical-align: middle" src="{{ registry.uri }}img/plus.png" /> Выбрать все</div>
			<div id="fm_unsel" class="btn"><img style="vertical-align: middle" src="{{ registry.uri }}img/minus.png" /> Снять выделение</div>
		</div>
	</div>
	<div style="float: right"><b>Размер:</b> <span id="fm_total">{{ totalsize }}</span></div>
</div>
...
<div class="fm_unsellabel">
	<div ondblClick="chdir('{{ part.id }}')"><img src="{{ registry.uri }}img/ftypes/folder.png" style="width: 50px{{ opacity }}" alt="[Каталог]" /></div>
...
<div class="fm_dirs_up" title="{{ part.name }}" style="text-align: center; cursor: pointer">
	<div ondblClick="chdir('{{ part.pid }}')"><img src="{{ registry.uri }}img/ftypes/folder.png" style="width: 50px{{ opacity }}" alt="[Каталог]" /></div>
...
<div class="fm_unsellabel">
	<a class="fm_pre" name="{{ part.name }}" id="fm_filename{{ i }}"><img src="{{ registry.uri }}{{ part.ico }}" style="height: 50px{{ opacity }}" alt="[Файл]" /></a>
	...
	<div id="fs_{{ part.id }}" style="color: green; font-weight: bold; {% if part.share %}display: block{% else %}display: none{% endif %}">Поделиться</div>
</div>
</div>
{% endfor %}
</div>
...
function delmanyrealConfirm() {
	$('<div title="Удалить">Вы действительно хотите удалить без возможности восстановления?</div>').dialog({
		buttons: {
			"Да": function() { delmanyreal(); $(this).dialog("close"); },
